...
SELECT
  ...
  (copies_count - donated_count - sales_count - problems_count - reservations_count - cart_items_count) > 0 AS is_available,
  (copies_count - donated_count - sales_count - problems_count - reservations_count - cart_items_count) as available_count,
...
FROM (
  SELECT
    ...
    COUNT(DISTINCT book_copy.id) FILTER (WHERE book_copy.donated_at IS NOT NULL) as donated_count
  FROM "Book" AS book
  LEFT JOIN (
    -- If the book copy has been returned or donated, we don't care about pending reservations, problems, etc
